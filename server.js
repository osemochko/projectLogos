...
server.post('/add-to-basket', (req, res) => {
  try {
    ...
    const indx = basket.findIndex(item => item.dish.id === req.body.dish.id); // you should change property "your_field" to your field name
    
    if (indx >= 0) {
      basket[indx].quantity += quantity;
    } else {
      basket.push({dish: req.body.dish, quantity}); // you should change property "your_field" to your field name
    }
...
server.post('/remove-from-basket', (req, res) => {
  try {
    ...
    const length = basket.length;
    const quantity = req.body.quantity || 1;
    const index = basket.findIndex(item => item.dish.id.toString() === req.body.dish_id.toString()); // you should change property "your_field" to your field id name
